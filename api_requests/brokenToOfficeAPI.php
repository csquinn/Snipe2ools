...
//Two requests are sent by brokenToOfficeAPI.php to SnipeIT. A put request updates everything besides being checked in or checked out, and a post request checks the asset out
...
try {
	...
	//rtd_location_id 15 = Office, status_id 7 = Broken
	//setting status_id = 7 should be redundant as Chromebook Managers set this status
	$response = $client->request('PUT', $snipe_url.'/api/v1/hardware/'.$id, [
		'body' =>'{"rtd_location_id":15,"asset_tag":"' . $serial .'","status_id":7,"model_id":' . $modelID . '}',
		'headers' => [
...
try {
	...
	header("Location: ../sites/brokenToOffice.php?SnipeRequestStatus=1". (($gSuccess == 0) ? '' : "&GoogleRequestStatus=".$gSuccess) ."&serial=". $serial);
