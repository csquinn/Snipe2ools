...
try {
	...
	$response = $client->request('PUT', $snipe_url.'/api/v1/hardware/'.$id, [
		'body' =>'{"asset_tag":"' . $serial .'","status_id":2, "model_id": '.$modelID	//model_id
		.((isset($location))?(', "rtd_location_id": '.$location):(''))	//rtd_location_id
		.'}',
		'headers' => [
...
header("Location: ../sites/workingToSchools.php?SnipeRequestStatus=1". (($gSuccess == 0) ? '' : "&GoogleRequestStatus=".$gSuccess) ."&serial=". $serial.
